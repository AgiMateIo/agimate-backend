...
SPRING_MAIL_HOST SPRING_MAIL_PORT APP_MAIL_FROM
...
# Mailpit from --profile infra: it accepts everything and delivers nothing, which is exactly what
# a confirmation letter needs locally. Read the letters at http://localhost:8025
[ -n "$SPRING_MAIL_HOST" ] || SPRING_MAIL_HOST="localhost"
[ -n "$SPRING_MAIL_PORT" ] || SPRING_MAIL_PORT="1025"
[ -n "$APP_MAIL_FROM" ] || APP_MAIL_FROM="no-reply@agimate.lc"
...
render() {
    ...
    sed \
        ...
        -e "s|__SPRING_MAIL_HOST__|$(esc "$SPRING_MAIL_HOST")|g" \
        -e "s|__SPRING_MAIL_PORT__|$(esc "$SPRING_MAIL_PORT")|g" \
        -e "s|__APP_MAIL_FROM__|$(esc "$APP_MAIL_FROM")|g" \
