...
public class DeviceApiService implements DeviceApi {
    ...
    private final DeviceApiServiceGrpc.DeviceApiServiceBlockingStub deviceApiStub;

    @Override
    public List<ConnectedDevice> getDevices(String userId) {
        try {
            log.debug("Calling device-api gRPC service - getDevices for userId: {}", userId);

            var request = GetDevicesRequest.newBuilder()
                    .setUserId(userId)
                    .build();

            var response = deviceApiStub.getDevices(request);
            ...
            throw mapGrpcException(e, "Failed to get devices from device-api");
        }
    }
    ...
    public List<DeviceTrigger> getTriggers(String deviceId) {
        try {
            log.debug("Calling device-api gRPC service - getTriggers for deviceId: {}", deviceId);

            var request = GetTriggersRequest.newBuilder()
                    .setDeviceId(deviceId)
                    .build();

            var response = deviceApiStub.getTriggers(request);
            ...
            throw mapGrpcException(e, "Failed to get triggers from device-api");
        }
    }
    ...
    public List<DeviceAction> getActions(String deviceId) {
        try {
            log.debug("Calling device-api gRPC service - getActions for deviceId: {}", deviceId);

            var request = GetActionsRequest.newBuilder()
                    .setDeviceId(deviceId)
                    .build();

            var response = deviceApiStub.getActions(request);
            ...
            throw mapGrpcException(e, "Failed to get actions from device-api");
        }
    }
    ...
    public void pushAction(String deviceAuthKeyId, Object data) {
        try {
            log.debug("Calling device-api gRPC service - pushAction for deviceId: {}", deviceAuthKeyId);
    ...
    private RuntimeException mapGrpcException(StatusRuntimeException e, String message) {
        ...
        return switch (code) {
            case UNAVAILABLE, DEADLINE_EXCEEDED ->
                new ServiceUnavailableStatusException(message + ": device-api service unavailable", e);
