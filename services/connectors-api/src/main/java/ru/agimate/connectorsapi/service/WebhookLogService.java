...
public class WebhookLogService {
    ...
    private void deliverWebhook(Webhook webhook, String eventType, Map<String, Object> eventPayload) {
        long startTime = System.currentTimeMillis();
        WebhookLog.WebhookLogBuilder logBuilder = WebhookLog.builder()
        ...
        if (eventPayload.get("credentialId") != null) {
            logBuilder.credentialId(UUID.fromString((String) eventPayload.get("credentialId")));
        }
        if (eventPayload.get("deviceId") != null) {
            logBuilder.deviceId((String) eventPayload.get("deviceId"));
        }
        ...
        saveLogRecord(logBuilder.build(), webhook.getId());
    }

    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void saveLogRecord(WebhookLog webhookLog, Long webhookId) {
        try {
            ...
            log.error("Failed to save webhook log record: {}", e.getMessage(), e);
        }
    }
}
