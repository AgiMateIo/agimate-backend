package ru.agimate.deviceapi.integration.telegram;
...
import ru.agimate.deviceapi.integration.IntegrationPlatformHandler;
import ru.agimate.deviceapi.integration.PlatformValidationResult;
...
public class TelegramPlatformHandler implements IntegrationPlatformHandler {
    ...
    @Override
    public void setupWebhook(Integration integration, Map<String, String> credentials, String webhookUrl) {
        String token = credentials.get("token");
        try {
            Map<String, Object> response = telegramApiClient.setWebhook(
                    token, webhookUrl, integration.getWebhookSecret());
            if (!Boolean.TRUE.equals(response.get("ok"))) {
    ...
    @Override
    @SuppressWarnings("unchecked")
    public TriggerRequest normalizeInbound(Integration integration, String rawBody) {
    ...
    @Override
    public Map<String, Object> executeTool(Integration integration, Map<String, String> credentials,
    ...
    @Override
    public boolean validateWebhookRequest(Integration integration, HttpServletRequest request) {
        String secretToken = request.getHeader(HEADER_SECRET_TOKEN);
        String webhookSecret = integration.getWebhookSecret();
