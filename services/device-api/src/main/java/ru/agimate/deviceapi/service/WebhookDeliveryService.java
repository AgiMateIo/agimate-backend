...
public class WebhookDeliveryService {
    ...
    public void deliverWebhook(Agent settings, TriggerLogAgent triggerLogAgent, TriggerRequest triggerRequest) {
        ...
        WebhookDeliveryLog.WebhookDeliveryLogBuilder logBuilder = WebhookDeliveryLog.builder()
                ...
                .requestUrl(settings.getWebhookUrl())
        ...
        try {
            ...
            if (settings.hasWebhookAuth()) {
                requestBuilder.header("Authorization", settings.getWebhookAuthHeader());
            }

            Request request = requestBuilder.build();

            log.debug("Delivering webhook to {} for trigger '{}'", settings.getWebhookUrl(), triggerRequest.name());
            ...
            try (Response response = httpClient.newCall(request).execute()) {
                ...
                if (response.isSuccessful()) {
                    log.info("Webhook delivered successfully: {} ({}ms, status: {})",
                            settings.getWebhookUrl(), duration, response.code());
                } else {
            ...
            logBuilder
                    ...
                    .durationMs(duration);
            log.error("Failed to deliver webhook to {}: {}", settings.getWebhookUrl(), e.getMessage());
            ...
            logBuilder
                    .errorMessage(e.getMessage())
                    .durationMs(duration);
            log.error("Unexpected error delivering webhook to {}: {}", settings.getWebhookUrl(), e.getMessage(), e);
        }
