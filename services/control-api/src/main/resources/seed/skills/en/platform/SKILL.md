...
- **LLM.** Your own providers: `list_llm_providers`/`get_llm_provider`/`create_llm_provider`/`update_llm_provider`/`delete_llm_provider`, `list_llm_provider_catalog` (exact baseUrl and models before creating), `refresh_llm_provider_models`/`list_llm_provider_models`; quotas `list/create/update/delete_llm_quota`; agent bindings `list_agent_llms`/`set_agent_llm`/`delete_agent_llm` (`set_agent_llm` creates or replaces a binding); `get_llm_usage` — spend and remaining quota per provider. The API key is stored encrypted and **never returned** — only the `apiKeyMask`.
...
- **Secrets never pass through you.** Connection credentials are deep-link only (see above). The agent key is likewise issued to the owner only through the `keyUrl` in the UI, never in a tool response. LLM provider API keys and `webhookAuthHeader` are accepted as input but never returned — don't expect them in tool results and don't ask the user for them in chat.
