...
| `INBOUND_RATE_LIMIT_VIEW_CALLS_PER_MINUTE` | Tool calls from connector views per minute per agent — `/manage/agents/{agentId}/views/tools/call` (default `120`, `<=0` disables) |
...
Единственное, чего не видно в сгенерированной схеме: чем «поле не прислали» отличается от «поле
очистили». В OpenAPI оба выглядят как необязательный nullable-параметр, поэтому договорённость
описана здесь и одинакова для всех `PATCH` в `/manage`:

| В теле | Что происходит |
|---|---|
| ключа нет либо `null` | поле не трогаем |
| непустое значение | записываем |
| пустая строка | **очищаем** (пишем `NULL`) |

Пустая строка выбрана вместо трёхсостоянийной обёртки (`JsonNullable`, `Optional`) по двум причинам.
Техническая: MVC у Spring Boot 4 работает на Jackson 3 (`tools.jackson`), а `jackson-databind-nullable`
— модуль Jackson 2, зарегистрировать его на бине `ObjectMapper` нельзя; голый `Optional` трёх
состояний не даёт вовсе — Jackson отдаёт `Optional.empty()` и на отсутствие ключа, и на явный
`null`. Содержательная: очищаемых полей во всём `/manage` четыре, все строковые, и для каждого
«пусто» и «нет значения» — одно и то же.
Собственная обёртка стоила бы десериализатора, `ValueExtractor` для Bean Validation и
`ModelConverter` для springdoc ради различия, которого у этих полей нет.

Поля, которые нельзя очистить (`name` у агента и команды), на пустую строку отвечают `400`.

Отдельно у `PATCH /manage/agents/{id}`: смена `type` с `WEBHOOK` на любой другой **сама** зануляет
`webhookUrl` и удаляет секрет `webhookAuthHeader` — иначе у агента остаются адрес и ключ, которые
уже никто не прочитает, а помнить об этом пришлось бы каждому клиенту. Обратный переход требует
`webhookUrl` — либо в теле запроса, либо уже сохранённого.

`PUT /manage/apps/{appId}` частичный с самого начала, поэтому `PATCH /manage/apps/{appId}` делает
ровно то же самое и добавлен только ради канонического глагола. Двух поведений здесь нет — второй
метод делегирует первому.
...
`extra_body` уровня провайдера и ссылку, где взять ключ. Пользователь дописывает только имя и
API-ключ.

`extra_body` в каталоге — маршрутизация шлюза, о которой пользователь не догадается спросить: у
OpenRouter это `provider.require_parameters` (не отдавать запрос хостеру, который молча выбросит
`tools`, `max_tokens` или `reasoning`) и мягкий порог скорости `preferred_min_throughput`. Как и модели,
это только предзаполнение: правка сида не доезжает до уже созданных провайдеров.
...
`updated_at`. Миграция таблицу только создаёт и не наполняет: INSERT-блок в changelog'е застывал бы
на том, чем инсталляцию засеяли впервые, и сдвинуть его могла бы только миграция данных.
Перезапись безопасна — в таблицу никто не пишет в рантайме, а фолбэк применяется по полям.

### Слои `extra_body`

Тело запроса к `/chat/completions` собирает `LlmCredentialsResolver` из трёх слоёв, каждый
следующий бьёт предыдущий (`ExtraBodyMerge`: объекты сливаются рекурсивно, скаляры и массивы
заменяются, `null` удаляет ключ — как в JSON Merge Patch):

```
дефолты реестра  <  extra_body провайдера  <  extra_body модели
```

Дефолт реестра сейчас один — лимит вывода. Без него хостер подставляет своё умолчание, и
reasoning-модель на маленьком умолчании тратит его целиком на рассуждения и заканчивает `length`,
не написав ответа. Значение — `min(max_output_tokens, 32768, context_window / 2)`: реестр хранит
лимит лучшего хостера листинга (бывает и сотни тысяч), а хостер, сверяющий `prompt + max_tokens`
с окном, отказывает всему запросу, — половина окна оставлена под промпт, размер которого здесь
неизвестен. Ключ выбирается по `supported_parameters` (`max_tokens`, иначе `max_completion_tokens` —
reasoning-модели OpenAI отвергают первый); без строки реестра, лимита или списка параметров не
шлётся ничего. Нужен ответ длиннее — `max_tokens` в `extra_body`, убрать дефолт — `"max_tokens": null`.

Дефолты получают только те, чьё тело уходит в `/chat/completions`: цикл агента (`CHAT`) и рутина
(`ROUTINE`, компакция). Медиа-назначения — нет: их тело может уйти в эндпойнт картинок или звука,
который лишнее поле отвергнет.
