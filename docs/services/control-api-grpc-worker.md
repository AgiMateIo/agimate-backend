...
| Field | Description |
|-------|-------------|
| `system_blocks` | Упорядоченные `PromptBlock` (stable-первые — prompt-cache): agent → инструкции → блоки `PromptBlockProvider`-коннекторов (memory) → team → skills-листинг → тела скиллов (DIALOGUE — все, SYSTEM_TRIGGER — подошедшие триггеру) → trigger guidance |
