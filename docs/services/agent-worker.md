...
| `ToolRegistry` | LLM name (`llm_name` from the backend, the worker's own sanitizing of `{namespace}.{name}` as the fallback) ↔ backend `(connector_code, name, connection_id, openWorld, material)`; schema parsing. Mutable within a run: a `LAZY` spec is routed but absent from `toolDefs()` until `disclose` applies a `describe_tools` delta; the loop re-reads `toolDefs()` every turn. |
| `ContextDelta` | The `describe_tools` result as the worker reads it (`{"tools": [proto-JSON specs], "unknown": [...]}`) and the compact `{"disclosed": [...]}` the model gets instead; applied by `ToolCallDispatcher` from the raw output, before the truncation cap, on a replay from the re-read output. |
