...
`prompt.tool-output-guidance`, `prompt.detached-tool-guidance`). Они уезжают в диалог, поэтому их
ось — язык диалога, а не язык инсталляции. Пока обе оси совпадают, это правильное место; когда
разойдутся (per-agent локаль), их придётся резолвить из рана. Ни `AgiMateAgent`, ни
`ContextBuilder` строк не хранят — они получают их из `ResponseTemplates`.
