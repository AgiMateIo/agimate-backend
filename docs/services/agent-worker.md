...
(`prompt.wrap-up`, отсюда и префикс `prompt.` вместо `notice.`): он уезжает в диалог, поэтому его
ось — язык диалога, а не язык инсталляции. Пока обе оси совпадают, это правильное место; когда
разойдутся (per-agent локаль), его придётся резолвить из рана. `AgiMateAgent` строку не хранит —
она приходит в конструктор от `AgentRunner`, так что сам цикл остаётся без захардкоженных текстов.
