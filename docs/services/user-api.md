...
| Эндпойнт | Что делает |
|---|---|
| `POST /auth/register` | Заявка на аккаунт + письмо. Ответ одинаков всегда |
| `POST /auth/register/confirm` | Подтверждение адреса: здесь заводится аккаунт, ответ — пара токенов |
...
не спотыкается об `uq_users_email`: письмо доказывает владение ящиком не хуже провайдера.
