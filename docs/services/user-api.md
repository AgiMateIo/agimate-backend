...
токен выписан (`auth_sessions.id`). Подписка на пуш-уведомления принадлежит входу, а не
пользователю, и claim — единственный способ узнать, какому именно: access-токен stateless и сам про
сессию не говорит. В токенах, выпущенных до появления claim'а, его нет, и это не ошибка. Не `sid`,
хотя в OIDC это ровно оно: в control-api `sessionId` — переписка, и короткое имя читалось бы как она.
