...
`POST /internal/notifications` закрыт ключом `intr` (заголовок `X-Internal-Auth-Key`, отдельная
цепочка фильтров): это не пользовательский токен, и роли из базы про сервис ничего не говорят.
