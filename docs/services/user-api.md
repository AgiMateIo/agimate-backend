...
Провайдеров у одного человека может быть несколько, и это работало всегда: вход через второго
провайдера находит аккаунт по подтверждённому адресу и присоединяется к нему. Пароль встал рядом
таким же полноправным способом, а здесь — то, что по адресу не решается, и управление списком.

| Эндпойнт | Что делает |
|---|---|
| `GET /auth/methods/` | Провайдеры (от старых к новым) и пароль, если он есть |
| `POST /auth/methods/link/{provider}` | Билет на привязку + готовый путь, куда вести браузер |
...
**Привязка идёт билетом, а не идентификатором в cookie.** Из четырёх cookie OAuth-флоу шифруется
только `oauth2_auth_request`; положи в соседнюю `user_id` — и её подделают, привязав чужой аккаунт
провайдера к любому пользователю. Поэтому клиент получает одноразовый непрозрачный билет
(`auth_tokens`, `purpose = PROVIDER_LINK`, 5 минут), билет едет в cookie, а **чей это аккаунт,
читается из базы**.

```
POST /user/auth/methods/link/GITHUB        → { ticket, authorizationPath }
GET  /user/oauth2/authorization/github?link_ticket=…   → провайдер → колбэк
                                           → редирект на redirect_to с ?linked=github
```

Ошибки приезжают тем же редиректом: `?link_error=invalid_ticket` (билет просрочен или потрачен),
`?link_error=already_linked` (этот аккаунт провайдера принадлежит кому-то другому),
`?link_error=provider_already_linked` (провайдер такого вида у аккаунта уже есть).
...
провайдера. До появления привязки случай был недостижим — вход связывал по адресу, а один адрес даёт
у провайдера один аккаунт.

**Адрес у провайдера в этом флоу не спрашивают вовсе.** Идентичность здесь — аккаунт, в котором
человек уже сидит, и именно поэтому привязка решает случай, который вход решить не может: GitHub на
другом ящике раньше молча заводил второй аккаунт. По той же причине `user_oauth_accounts.email`
стал nullable — провайдер, отдающий только идентификатор, теперь законен.
...
- `auth_tokens` — одноразовые секреты: письма о пароле (`PASSWORD_RESET`) и билеты привязки
  провайдера (`PROVIDER_LINK`)
