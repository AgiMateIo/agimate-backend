...
живёт в `pending_registrations` — пароль там уже хешем, — и `users` продолжает означать «адрес
...
- `pending_registrations` — заявки, не дошедшие до подтверждения (пароль хешем, не текстом)
