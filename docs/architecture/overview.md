...
ложится в `updates/YYYY/MM/DD-NN-name.xml`; `updates/` — накопитель, а не летопись, и периодически
сворачивается обратно в `initial/`, поэтому `initial/` читается как текущая схема целиком.
Правила именования индексов и ограничений, вид комментариев и порядок сворачивания —
[decisions/schema-conventions.md](../decisions/schema-conventions.md).
