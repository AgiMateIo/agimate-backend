...
- **Ссылка навыка** (`agent_skill_connections`, на связке агент↔навык) — *какой именно* инстанс имеет
  в виду навык: `(agent_skill_id, connector_code) → connection_id`, уникально. Доступа она не даёт —
  это ответ на вопрос «какой из двух телеграмов», которого раньше не существовало: внутренние
  ...
  для внешних выбор обязателен. Отсюда признак **«навык удовлетворён»** = у каждого объявленного кода
  ...
  Объявленный код **без своей ссылки** разрешается во все привязанные инстансы этого кода. Это не
  временная мера под легаси: состояние возникает всякий раз, когда автор добавляет коннектор в уже
  привязанный навык, — и решено оно так же, как было до ссылок, чтобы работающий агент не сузился
  молча. Цена известна: для внешнего коннектора с двумя инстансами навык получит оба, а статус покажет
  первый. Сузить это до «фолбэк только для внутренних» — открытое решение, сознательно не принятое.
