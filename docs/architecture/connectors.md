...
- **Ссылка навыка** (`agent_skill_connections`, на связке агент↔навык) — *какой именно* инстанс имеет
  ...
  Для привязок, сделанных до появления ссылок, код разрешается во **все** привязанные инстансы этого
  кода — ровно прежнее поведение, чтобы работающие агенты не сузились молча.
