...
`env.agentId → team → board`; agents: `env.agentId → team`), «правила нет» — ресурс либо явно
адресован в вызове (сессия у webchat/acp, экземпляр у telegram/mcp), либо отсутствует (media).

Командные коннекторы независимы: у команды может быть подключён только `board`, только `agents`,
оба или ни один, и участие в каждом — его собственная привязка. Общее у них одно — круг, и он
считается в одном месте: `service/team/TeamCircleService` (`isMember`, `roster`, `participants` —
состав, суженный привязкой к подключению и пушем, той же выборкой, что у роутера). Коннектор, которому
нужен круг команды, берёт его оттуда, а не заводит свою проверку членства.
...
- **Binding** (`agent_connections`, M:N agent↔connection) — гейт доступности. Нет активной строки →
  коннектор агенту недоступен (даже если `connections`-запись есть). Выдаёт и отзывает его **только
  пользователь** — для любого вида коннектора: внешний адресуется id экземпляра, внутренний кодом
  (строка-режим материализуется на месте, `ensureModeConnection`). Скиллы привязок не создают и не
  снимают: они лишь объявляют, *с каким* экземпляром работают, и остаются неудовлетворёнными, пока он
  не открыт. Привязки, удерживаемые активным каналом (webchat/acp), создаются каналом и уходят с ним.
- **Ссылка навыка** (`agent_skill_connections`, на связке агент↔навык) — *какой именно* инстанс имеет
  в виду навык: `(agent_skill_id, connector_key) → connection_id`, уникально. Ключ — это ключ требования
  из объявления навыка (`skills.connectors`, JSONB; `ConnectorRequirement`): по умолчанию равен коду, а
  отличается, когда навыку нужны два экземпляра одного коннектора (два MCP-сервера). Доступа ссылка не
  даёт — это ответ на вопрос «какой из двух телеграмов», которого раньше не существовало: внутренние
  привязывались сами (инстанс один на пользователя), внешние навык привязать не мог вовсе и молча не
  работали. Для внутренних код заполняет сервер (`ensureModeConnection`, легальный кандидат один);
  для внешних без выбора строки нет — навык привязывается неудовлетворённым (так создаётся агент из
  пресета), и закрывает его мастер или фолбэк по идентичности. Отсюда признак **«навык удовлетворён»** = у каждого объявленного ключа
  есть **хотя бы один** пригодный инстанс: привязан к агенту, авторизация жива и у DYNAMIC-коннектора
  он принёс хоть что-то — тул или триггер. **Неудовлетворённый скилл агенту не отдаётся вовсе** — иначе его тело обещало бы тулы, которых
  в контексте нет, — но с версии 2026-09-13 он не исчезает молча: в каталоге `<skills>` остаётся
  строка со `status: unavailable` и `blocked_by` (`AgentSkillService.RequirementState`:
  `NOT_CHOSEN`, `NOT_BOUND`, `UNAUTHORIZED`, `NO_CAPABILITIES`, `UNKNOWN_CONNECTOR`), чтобы агент мог сказать
  пользователю, чего ему не хватает. Ту же причину читают мастер привязки (`SkillConnectorStatus.state`)
  и `platform.list_agent_skills` — считается она один раз, в `AgentSkillService.gate`. Тул-гейт `RunContextService` работает по
  конкретным коннекшенам этих скиллов, а не по кодам: два телеграма больше не протекают друг в друга.
  Объявленный ключ **без своей ссылки** разрешается по идентичности, когда требование её несёт
  (`params` → `IntegrationConnectorHandler.identifierOf` → `sub_code`; у MCP это URL сервера), и во
  все привязанные инстансы кода, когда не несёт. Это не временная мера под легаси: состояние возникает
  всякий раз, когда автор добавляет коннектор в уже привязанный навык, — и решено оно так, чтобы
  работающий агент не сузился молча. Цена известна: для внешнего коннектора без идентичности и с двумя
  инстансами навык получит оба, а статус покажет первый.
- **Требование навыка** несёт больше, чем код (решение —
  [../decisions/skill-connector-requirements.md](../decisions/skill-connector-requirements.md)):
  `params` — несекретные поля формы интеграции, которыми мастер подключения предзаполняет создание
  экземпляра; `tools`/`triggers` — правила доступа (`allow` xor `deny`). Правила **советующие**:
  `SkillPolicySync` пишет их строками `agent_connection_policies` с `source = skill:<id>` при привязке
  навыка, замене ссылок и `refresh` (для последнего это сброс к правилам автора), снимает при отвязке;
  чужая строка на том же `(kind, name)` — конфликт, возвращается в статусе и не трогается; `PATCH`
  правила пользователем стирает `source`. План привязки для мастера —
  `GET /manage/agents/{id}/skills/plan?skillId=`: по требованию — подходящие коннекции (по идентичности,
  иначе все экземпляры кода), поля формы, строки правил; порядок шагов мастера — создать коннекцию →
  привязать к агенту → привязать навык, потому что правила ложатся на существующую привязку.
...
- **`<Name>ToolService`** — методы с собственной MCP-совместимой `@Tool` (`name`/`title`/`description`/
  ...
  (`type`, `intervalSeconds`/`cron`/`zone`, `spreadSeconds`, `timeoutSeconds`), на материализации экземпляра reconcile-синк
...
обращением (`probe`/`tools/list`/`tools/call`, единый чокпоинт `McpClient.openSession`) и, что важнее,
внутри резолвера имён самого клиента. Флаг `app.net.allow-private-targets` (default `false`) снимает
...
где Spring-бинов ещё нет. Русский первоисточник остаётся в коде фолбэком.

**Декларации тулов пишутся по-английски и бандла не имеют** — `@Tool(description)`, `@ToolParam`
(включая значения-примеры внутри них) и `TriggerSpec.description`. Это стандарт, а не текущее
состояние: их читает модель, а не пользователь, поэтому русский в декларации остался бы русским и на
EN-инсталляции.

`@Tool(title)` **не используем**. Единственным его пользователем был `sheets` (12 русских подписей,
уходивших и в UI-листинги, и агенту через `ConnectorToolSpec.title`) — при 57 тулах без подписи это
делало листинг разнородным, а сами подписи были единственным русским в слое. Подписи тулов для UI —
отдельная нерешённая задача: если они нужны, то у всех коннекторов и с переводом, а не у одного.

Если когда-нибудь понадобится переводить сами декларации: `BaseConnectorHandler` строит `toolSpecs` в конструкторе через
`ToolSchemaReflector` (`@UtilityClass`, статические методы), поэтому язык надо протаскивать либо в
конструкторы коннекторов, либо в резолв у потребителей (`ToolDefinitionService`, `RunContextMapper`,
`AcpSessionRegistry`). Полезное уточнение к оценке: `@ToolParam` читается в рефлекторе **в одном
месте** — дальше по рекурсии `schemaFor` идёт уже готовая строка.

Единственный путь, которым русский текст тула может дойти до пользователя, — пересказ
`get_connector` platform-агентом, и английский `platform`-скилл велит ему пересказывать, а не
цитировать.
...
**Разброс cron'а (`@Job(spreadSeconds)`).** Cron прикалывает строки всех подключений установки к
одной секунде, и она просыпается разом — заметно на CPU, а у задач с раном модели ещё и на токенах.
Поэтому у задачи, которой важен темп, а не момент («ночью», «раз в час»), объявляется окно, и
экземпляр выбирает внутри него свою секунду: `ConnectorRegistry.declaredJobs` — единственное место,
через которое идут оба писателя строк, — считает смещение от `connections.id` и подставляет в
`config.cron` уже сдвинутое выражение, оставляя объявленное в `config.baseCron`. Шедулер о разбросе
не знает: он по-прежнему разбирает то, что лежит в `config.cron`.

Смещение **детерминированное**, а не случайное: стартовый ресинк перечитывает декларацию, и
разыгранное заново уводило бы задачу по окну на каждом деплое. Окно применимо только к выражению,
называющему точку в периоде (целые секунда/минута/час, окно внутри тех же суток) — остальное падает
на старте в `ConnectorBootstrap`. Момент, который обещан человеку («сводка в 9:00») или продиктован
внешним миром, окна не получает — `spreadSeconds` по умолчанию `0`. Динамические задачи агента
(`time.schedule`) собирают `JobSpec` руками и разбросу не подлежат по построению.
...
- `ConnectorIdentityListener` (AFTER_COMMIT) превращает их в **декларативные** строки `connector_jobs`
  ...
  Планирование ресинк не трогает — с одним исключением: у CRON-строки в состоянии `PENDING` и не на
  паузе, чьё выражение в декларации изменилось, `next_run_at` пересчитывается. Висящий дедлайн
  посчитан по выражению, которого больше нет, и без пересчёта строка отработала бы по нему ещё раз —
  для задачи, только что получившей окно разброса, это вся установка, просыпающаяся вместе в
  последний раз.
