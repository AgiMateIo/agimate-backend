...
Для каждого получателя `ChannelRouteResolver.resolve` отдаёт один из четырёх исходов: `DIRECT`
(канала нет), `CHANNEL` (канал и извлечённое сообщение), `SKIP` (канал есть, но триггер отфильтрован),
`CANCEL` (сообщение оказалось стоп-командой).
...
**Стоп-команда.** Текст входящего, точно совпавший с `/stop` (переопределяется ключом `stopCommand`
в конфиге канала, пустое значение выключает), даёт `CANCEL`. Распознавание живёт здесь, а не в
хендлере, и поэтому работает для любого канала с текстом: телеграму оно нужнее всех — у него нет
другого провода до платформы, — но ничего телеграмного в нём нет. Совпадение точное, не префиксное:
съесть сообщение, на которое агент должен был ответить, хуже, чем не поймать стоп.

Сессия при этом ищется **только существующая живая** — остановка не должна заводить разговор,
чтобы было что останавливать.

Роли в `Channels`: `prompt` — всегда канал входящего; `progress` — тот же канал, если хендлер
объявил `deliverProgress` (webchat, ACP); `answer` у входящего не заполняется — доставка сама падает в
`prompt`. `answer` заполняют раны без входящего: напоминание (проактивный случай ниже) и события,
продолжающие разговор (`Channels.continuation()` у `tool_completed` и отчёта субагента).

**Адрес ответа.** `ChannelInfo.address` — куда внутри канала уходит ответ, в том виде, как его читает
хендлер: `handler.replyAddress(config, trigger)` у Telegram даёт `{chatId}`, у `generic` — только те
поля `trigger.data`, на которые ссылается `replyToolParams`. Webchat, ACP и субагенты адресуют
сессией и адреса не заводят. Адрес получает и `CANCEL`: ответ «нечего останавливать» уходит в чат,
приславший команду. Раны событий в разговоре (`tool_completed`, отчёт субагента, напоминание)
копируют снимок каналов вместе с адресом, поэтому отвечают туда же, откуда пришёл разговор. Почему
адрес в снимке, а не в журнале сессии — [`../decisions/reply-address-in-channels.md`](../decisions/reply-address-in-channels.md).
...
уйдёт сообщение. Адрес объявленного `ChannelInfo` переносится как есть: `time.schedule` сохраняет
адрес вызвавшего рана в аргументах джобы, и напоминание приходит в тот чат, из которого его поставили.
...
Маршрут `CANCEL` рана **не создаёт вовсе**: ни строки, ни слота воркера, ни записи в истории —
команда адресована платформе, а не агенту, и в его контекст попадать не должна. Вместо этого
останавливаются живые раны сессии (`RunCancellationService.cancelSessionFromChannel`). Ответ в чат
уходит, только если останавливать было нечего; иначе ответом служит собственное «Остановлено…»
остановленного рана, и оно содержательнее. Подробности — [`../decisions/run-cancellation.md`](../decisions/run-cancellation.md).

---

## 3. sessionId и очередь

Сессия есть у **любого** рана, и `DbosTransport` кладёт ран в партиционированную очередь
`agent_exec` с `partitionKey = sessionId`. Порядок разрешения — в `TriggerRouterService`:

1. ран порождён раном (`tool_completed`) → сессия родителя;
2. есть канал → его сессия (prompt, иначе answer) — то же выражение `Channels.sessionId()`, которым
   гасит раны стоп-команда из канала;
3. иначе → живая сессия `(agent, connection)`, `scope = CONNECTION`, заводится по месту.

Отсюда следствия:

- раны одной сессии исполняются строго по одному — **single-writer-per-session это свойство
  транспорта**, а не блокировка в коде;
- события одного коннекшена больше не идут параллельно: они встают в очередь и схлопываются
  стирингом. Обратная сторона — очередь, поэтому глубина логируется предупреждением;
- «канальность» рана определяется каналами в снапшоте, а не наличием сессии: канальная проекция
  сообщений и история — только у рана с каналом ([`../decisions/agent-sessions.md`](../decisions/agent-sessions.md)).
...
1. получает целевой `ChannelInfo` роли, резолвит сессию (указанную либо активную), сверяет владение
   каналом;
2. отдаёт хендлеру `OutboundDispatch.address` — адрес из того же `ChannelInfo`. Снимку, созданному до
   появления адресов, на один выпуск остаётся старый путь: `trigger_input` последней входящей строки
   сессии. Поэтому `trigger_input` пишется только у рана с prompt-каналом — событие не должно
   становиться «последним входом»;
...
|---|---|
| `generic` | data-driven: всё поведение из `config`. Покрывает динамические коннекторы (`app`), где триггеры и тулы объявляет само приложение и хендлер в коде написать нельзя |
...
- `{trigger.<dot.path>}` — поле из адреса ответа (пути считаются от `trigger.data`). В адрес при
  маршрутизации попадают ровно поля, упомянутые в шаблоне, поэтому шаблон, поменявшийся после создания
  рана, может сослаться на поле, которого в снимке нет.
...
Отправка по снимку без адреса — ран, за которым не стоит ни одно входящее сообщение, — оставит все
`{trigger.*}` пустыми: reply-тул упадёт уже в коннекторе, если шаблон зависит от данных входящего
события. Это известное ограничение модели, а не баг рендерера.
...
- **Отдельного канала ответа у входящего нет** — `answer` заполняется только у ранов без prompt, а
  ответ на сообщение всегда уходит в канал, откуда оно пришло.
- **`ChannelInfo.messageId`** зарезервирован под треды и реплаи, сейчас не заполняется.
- **Одна сессия на бота, а не на чат.** Адрес в снимке рана не даёт ответу уйти в чужой чат, но ран
  из чата 1, поглотивший стирингом сообщение из чата 2, ответит только в чат 1.
