...
> Раздел описывает архитектуру на июнь 2026 и с тех пор разошёлся с кодом: строку коннектора заводит
> `ConnectorBootstrap` из декларации хендлера, а не `INSERT` миграцией; `integration_credentials`
> поглощены `connections` + `secrets`; эндпойнты подключений переехали в `/manage/connections/*`.
> Выводы про аддитивность остаются в силе, конкретные механизмы — нет.
