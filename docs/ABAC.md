...
## 10. Roadmap: эффекты политик за пределами ALLOW/DENY

Сейчас `effect ∈ {ALLOW, DENY}` — статичное решение до вызова. Идеи на будущее (не реализованы):

- **`ASK`** — не разрешать/запрещать статично, а спросить у пользователя в момент вызова. Открытый
  вопрос — *как* спрашивать в разных каналах: в ACP/IDE есть `session/request_permission` (готовый
  механизм), в webchat — inline-кнопки, в telegram — reply-клавиатура, а для direct/scheduled-ранов
  спрашивать некого (тогда `ASK` вырождается в DENY или timeout-DENY). То есть `ASK` требует
  канал-специфичного «запросчика подтверждения» и политику поведения при его отсутствии.
- **`LLM_DECISION`** — отдельный вызов LLM с вопросом «нормально ли вызвать эту тулу с такими
  аргументами в этом контексте», решение — из его вердикта. Дороже (лишний вызов на каждый tool-call
  под этой политикой), но ловит то, что не выразить статичным правилом (напр. «переводы до 1000 ₽
  можно, выше — нельзя»). Открытые вопросы: какая модель судит, что в контексте вердикта, аудит
  решений, защита самого судьи от prompt-injection из аргументов.

Первое приближение к `ASK` уже частично есть в ACP-фазе 3/4: мутирующие IDE- и MCP-тулы спрашивают
подтверждение через `session/request_permission` (но это захардкожено по MCP-аннотации `readOnly`, а
не выражено политикой `ASK`).