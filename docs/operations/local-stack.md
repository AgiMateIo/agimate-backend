...
Про пуши скрипт не спрашивает — генерировать там нечего, креды берутся из консолей RuStore и
Firebase. Кладутся тем же способом: `APP_PUSH_RUSTORE_PROJECT_ID`, `APP_PUSH_RUSTORE_SERVICE_KEY` и
`APP_PUSH_FCM_CREDENTIALS` (base64 с JSON сервисного аккаунта) в `services/.env`, дальше `--force`;
уезжают они в user-api, который владеет устройствами и транспортом. Пусто — подписки регистрируются,
уведомления по этому каналу не уходят; каналы независимы, настроенный работает без второго.
