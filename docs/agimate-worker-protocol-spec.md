...
> Версия: **v2.1a** (v2 реализована; этапы: GetRunContext → SaveMessage → тонкий payload; v2.1 — структурные tool-ходы в истории; v2.1a — tool-ход дробится на две записи: вызов до исполнения, результаты после). Принципы v2: бэкенд собирает контекст и владеет политикой, воркер рендерит и крутит цикл; воркер — единственный писатель истории, доставка — проекция записи; воркер знает только `{agent_id, run_id}`.
>
...
>
> **v2.1a (раздельная фиксация вызова и результатов).** Раньше `tool_turn` (вызовы + результаты) уходил одной записью PROGRESS/TOOL_CALL уже **после** исполнения тулов — канал молчал всё время работы. Теперь ход — две записи: PROGRESS/TOOL_CALL с `tool_turn{text, calls[]}` шлётся **до** dispatch (канал сразу видит «🔧 name», это же `tool_use`-половина истории), а PROGRESS/TOOL_RESULT с `tool_turn{results[]}` — после, с пустым `text` (в канал не доставляется, история-only). `GetRunContext.history` отдаёт обе как соседние записи; воркер сшивает их в нативную пару look-ahead'ом по порядку. Осиротевшую results-запись (calls-половину срезало окном истории) воркер отбрасывает — `tool_result` без `tool_use` провайдеры отклоняют. Легаси-раны с `calls+results` в одной записи по-прежнему читаются.
...
- `LlmCredentials.extra_body_json` — JSON-объект доп. полей тела chat/completions (расширения
  ...
  только набор ключей.
- `LlmCredentials.input_modalities` — модальности входа модели из реестра
  (`llm_provider_models`, фолбэк `llm_model_defaults` влит write-time). Воркер решает по ним
  **на каждый вызов**, подавать ли image-вложения инлайном (`Media`): нет `image` → байты не
  тянутся, вложение остаётся текстовым стабом, и в промпт добавляется system-подсказка «картинки
  тебе не видны — используй media.read_image по id» (симметричная подсказка «видишь напрямую» —
  когда вложения поданы). Пусто = модель реестру неизвестна → оптимистично прикладываем
  (поведение до этого поля). Стабы вложений на бэке (`MediaStubs`) нейтральны к видимости —
  рамку задаёт только воркер.
