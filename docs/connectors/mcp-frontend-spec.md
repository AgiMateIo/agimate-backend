...
Отдаёт закэшированные (после `test` / создания) тулы конкретного MCP-сервера — для отрисовки и для
выбора при выдаче политик. Для не-MCP интеграции вернёт пустой список.
