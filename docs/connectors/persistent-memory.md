...
Вью `ui://persist-memory/memory` (страница — `resources/views/persist-memory/memory.html`,
решение — [../decisions/connector-views.md](../decisions/connector-views.md)) открывает владелец агента
со страницы агента. Панель показывает cold с версией и список заметок, даёт править cold и добавлять
заметки. Новых тулов у неё нет: она зовёт те же четыре тула от имени агента, поэтому у них видимость
`{MODEL, VIEW}`, а чтение ссылается на панель через `@Tool(view)`.

- Правка cold идёт через CAS `update_memory` с версией, прочитанной при открытии: если агент переписал
  память раньше, панель получает конфликт версии и просит обновить, правка остаётся в поле.
- Заметки панель не удаляет и не правит: это сырьё консолидации, а партия может быть уже захвачена
  джобой. Заметка из панели — без сессии.
