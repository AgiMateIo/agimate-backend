...
- `consolidation` (PERIODIC, 86 400 с) — single-flight: если консолидация уже идёт
  ...
  Раз в сутки, потому что несвёрнутые заметки и так доходят до контекста агента отдельным блоком:
  консолидация лишь держит этот блок коротким, а запомненным факт делает сама заметка.
