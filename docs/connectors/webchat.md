...
Под `/manage/webchat` живёт **только транспорт** — старт чата, отправка в него, токен на его
живой канал, список контактов. Сама переписка (листинг, переименование, закрытие, история,
отметка прочитанного) — общий ресурс `/manage/sessions`, один на все каналы: сессия Telegram и
сессия вебчата отличаются транспортом, а не тем, чем является переписка.
...
- явная отметка `POST /manage/sessions/{id}/read` (тело `{lastReadMessageId}`; без тела —
  до конца сессии);
...
`GET /manage/sessions/` и `GET /manage/webchat/contacts/` отдают, кроме самой строки, три
вычисляемых поля: `unreadCount`, `lastMessage` (превью, обрезанное до 160 символов; `progress` не
превью) и `isRunning`. Все три считаются по `webchat_messages`, поэтому у сессии другого коннектора
приходят нулями: непрочитанное внешнего мессенджера ведёт он сам, а не мы.
...
`GET /manage/sessions/{id}/messages/` выбирает таблицу по коннектору сессии и приводит обе к одной
форме: у канальной записи `messageId`/`parts` приходят `null`, а `kind` становится `stream`
(`answer`/`progress`/`error`), `INBOUND` — направлением `USER`. Раздвоение хранилища клиенту не
видно намеренно: день, когда таблицы схлопнутся, не должен ломать API.
