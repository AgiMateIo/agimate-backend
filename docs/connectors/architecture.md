...
**Слою тулов бандл не нужен**: `@Tool(description)`, `@ToolParam` и `TriggerSpec.description` уже
написаны в коде по-английски. Единственное исключение — `@Tool(title = …)` в `SheetsToolService`:
12 русских подписей, которые уходят и в UI-листинги тулов, и агенту (`ConnectorToolSpec.title` по
gRPC). При этом `title` вообще задаёт только sheets — у остальных 57 тулов его нет, и они падают на
фолбэк к `name`. Так что это локальная несогласованность одного коннектора, а не пробел локализации,
и решать её надо продуктовым вопросом: нужны ли человеческие подписи тулов вообще (тогда для всех
коннекторов и с переводом) или нет (тогда убрать и из sheets).

Если понадобится переводить: `BaseConnectorHandler` строит `toolSpecs` в конструкторе через
