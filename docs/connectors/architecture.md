...
**Описания тулов и параметров не локализованы** — `@Tool(description)` и `@ToolParam` остаются
русскими при любом значении проперти. Их читает LLM как справку по вызову, а не пользователь, и цена
другая: `BaseConnectorHandler` строит `toolSpecs` в конструкторе через `ToolSchemaReflector`
(`@UtilityClass`, статические методы), поэтому язык надо протаскивать либо в конструкторы всех
коннекторов, либо в резолв на стороне потребителей (`ToolDefinitionService`, `RunContextService`,
`AcpSessionRegistry`). Это правка ядра SPI — делать её нужно отдельным осознанным заходом.

`TriggerSpec.description` бандла не требует: эти описания уже написаны в коде по-английски. Единственный
путь, которым русское описание тула может дойти до пользователя, — пересказ `get_connector`
platform-агентом, и английский `platform`-скилл велит ему пересказывать, а не цитировать.
