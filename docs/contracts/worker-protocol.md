...
- **Смена формы чекпоинта** (DBOS-payload, выходы шагов) несовместима с in-flight ранами → деплой
  ...
  (`save_message` на каждую), `GetTurn` обязан вернуть в том же виде — включая флаг `thinking`.
...
`GetTurn(agent_id, run_id, turn_index)` → `GetTurnResponse{role, text, thinking, tool_calls,
tool_results}` — зеркало `SaveTurnRequest` без телеметрии и без `thinking_text` (цикл рассуждение
не читает). Дословно и без капов: реплей повторяет `ExecuteToolAsync` с этими аргументами, а бэк
сравнивает их с записанными. `thinking` = «`thinking_text` не пуст» — от него зависит, будет ли у
хода строка 💭 в канале, то есть число durable-шагов. `NOT_FOUND`, если хода нет: для реплея это
...
Оно же стоит в листинге отложенных тулов, в аргументах `describe_tools` и в журнале ходов, поэтому
...
но не отдаёт модели в `tools`, пока результат `describe_tools` не принесёт полную спеку. Маркер
