...
| `MessageLog` | `SaveMessage`, `SaveTurn`, `GetTurn`, `SavePrompt` |
...
Контекст рана собирается **один раз** в начале (`GetRunContext`) и дальше живёт в памяти
воркфлоу — в durable-шаг он намеренно не заворачивается. Внутри одного исполнения конфигурация
не может поменяться под ногами по ходу рана, поэтому versioned reads не нужны.

Плата — реплей после краша: он не восстанавливает контекст из чекпоинта, а собирает заново. Ран
получит сегодняшний контекст, а не тот, с которым начинался (история успела пополниться, настройки
могли отредактировать), и упадёт, если агента к этому моменту выключили. Обмен осознанный: чекпоинт
`prepare_context` хранил весь диалог целиком — системный промпт, сообщение пользователя, заметки
памяти и всё окно истории, — то есть делал системную базу DBOS вторым хранилищем переписки. Реплеи
случаются только при падении процесса на середине рана.

Тот же принцип у остальных шагов рана: **чекпоинт хранит ссылку, данные — у control-api**
([decisions/dbos-ids-only.md](../decisions/dbos-ids-only.md)). Шаг `llm_call` возвращает
`call_id`, индекс хода, `finish_reason`, модель и числа токенов — ход ассистента он записывает
`SaveTurn`'ом до фиксации чекпоинта, и реплей читает его обратно `GetTurn`'ом. Шаг `tool_calls`
возвращает id и статусы вызовов; вывод и текст ошибки реплей перечитывает `GetToolResult`'ом,
interim детача и нотисы таймаута регенерирует. В обычном пути ни того ни другого не происходит —
результат шага остаётся в памяти рана.

Отсюда два жёстких правила:

- **Смена формы чекпоинта** (DBOS-payload, выходы шагов) несовместима с in-flight ранами → деплой
  только после **drain**: остановить триггеры, дождаться пустых очередей DBOS. Снятие или
  добавление durable-шага — тоже смена формы: шаги сверяются по паре (`function_id`,
  `function_name`), и сдвиг нумерации ломает реплей уже начатых ранов
  (`DBOSUnexpectedStepException`). Поэтому всё, из чего воркер считает строки прогресса
  (`save_message` на каждую), `GetTurn` обязан вернуть в том же виде — включая `thinking_text`, из
  непустоты которого берётся строка 💭.
...
`SaveTurn` и `SavePrompt` чекпоинтов не добавляют, поэтому drain перед их изменением не нужен.
...
(`ContextSpec`: `DIALOGUE` при prompt-канале в снапшоте `agent_runs.channels`; `DIALOGUE_EVENT` у
события, продолжающего разговор, — `tool_completed` или триггер с `continuesConversation`; иначе
`SYSTEM_TRIGGER`; таблица — в [agents-and-runs.md](../architecture/agents-and-runs.md)) целиком на
бэке, воркер рендерит блоки в присланном порядке.
...
`GetLlmCredentials` вызывается внутри каждого шага `llm_call`, вне чекпоинта.
...
## Журнал: `SaveMessage`, `SaveTurn`, `GetTurn`, `SavePrompt`
...
полем на сообщении, потому что возвращается провайдеру следующим запросом: почему так —
[../decisions/reasoning-content.md](../decisions/reasoning-content.md).
...
`finish_reason`/`model`/`call_id` — provenance LLM-хода; `call_id` чеканит воркер
(`run_id-порядковый номер вызова в ране`, одинаковый на реплее) — это join-ключ к
`llm_usage_log.call_id` и зерно для id вызовов тулов.

Ход `ASSISTANT` пишется внутри шага `llm_call`, до фиксации его чекпоинта: чекпоинт хранит только
`turn_index`, и реплей после падения читает ответ модели отсюда.

### `GetTurn`

`GetTurn(agent_id, run_id, turn_index)` → `GetTurnResponse{role, text, thinking_text, tool_calls,
tool_results}` — зеркало `SaveTurnRequest` без телеметрии. Дословно и без капов: реплей повторяет
`ExecuteToolAsync` с этими аргументами, а бэк сравнивает их с записанными. `thinking_text` едет
целиком, а не флагом: реплей пересобирает из хода то же сообщение, что ушло провайдеру, а провайдер в
режиме размышления требует своё рассуждение обратно, когда в запросе есть `tools`. Из непустоты этого
же текста берётся строка 💭, то есть число durable-шагов. `NOT_FOUND`, если хода нет: для реплея это
инфраструктурная ошибка рана (`SaveTurn` best-effort, и запись могла не дойти).

Поле 3, прежний `bool thinking`, **пока пишется** — его читает воркер предыдущего релиза, а строка 💭
у него durable-шаг: снять поле в тот же выкат значило бы отнять шаг у рана, начатого до выката, и
сломать его реплей. Порядок выката поэтому control-api → воркер, drain не нужен: старый воркер видит
привычный флаг, новый читает текст. Зарезервировать поле — релизом позже, когда читающих не осталось.

**Транскрипт и учёт.** Ход `ASSISTANT` пишется на каждый вызов, на который модель ответила, —
внутри шага `llm_call`, до того как цикл решит, что с ответом делать. Поэтому пустой ответ, который
цикл выбрасывает и переспрашивает, и обрыв по truncation в журнале есть (пустая или неполная
строка); из истории следующих ранов их выкидывает сборка (`RunHistoryAssembler`: ход без текста и
вызовов пропускается, прерванный ран в историю не попадает). Usage (`ReportLlmUsage`) считается на
**каждый** дошедший до модели вызов, включая те, что закончились ошибкой провайдера, — у них хода
нет. И наоборот: входящий ход LLM-вызовом не порождён и парной usage-строки не имеет. Учёт покрывает
весь расход, журнал — транскрипт; `agent_run_turns` не является полным перечнем LLM-вызовов.
...
`tool_call_id` **чеканит воркер** (`LlmMessageMapper.mintToolCallId`), а не модель: id из ответа
провайдера — данные, а не ключ. Часть OpenAI-совместимых серверов нумерует вызовы позиционно
(`call_0`, `call_1`), и счётчик сбрасывается на каждом ответе — тот же id возвращается с другим
входом и упирается в ключ идемпотентности `(agent_id, external_id)`, который живёт всё время жизни
агента. Свой id выводится из id воркфлоу LLM-вызова (replay чеканит те же id) и имеет вид
`[a-zA-Z0-9]{9}` — пересечение требований провайдеров: Mistral не принимает ничего другого, OpenAI
ограничивает длину 40 символами. Провайдерский id никуда дальше маппера не уезжает.

Девяти символов (62⁹ ≈ 1,4·10¹⁶) хватает **потому, что адрес вызова — пара `(agent_id,
external_id)`**: коллизия в пределах вызовов одного агента недостижима. Искать вызов по одному
`external_id` на такой длине уже нельзя, поэтому агент в `ToolCallLogService` — часть адреса, а не
проверка после глобального поиска.
...
| `ABORTED` | Тот же `tool_call_id` переиспользован под другой вызов — другой тул либо другой вход |
...
родителя без prompt (`DIALOGUE_EVENT`-контекст: все навыки разговора, без trigger guidance). Бегущий main заберёт его стирингом, свободная
...
**LLM-имя считает бэк.** `llm_name` в спеке — имя, под которым модель видит и вызывает тул
(`platform__agent_list`: `{namespace}.{name}` после санитизации, коллизии разрешены суффиксом).
Оно же стоит в листинге отложенных тулов, в аргументах `load_tools` и в журнале ходов, поэтому
одно на всех. Пустое `llm_name` (старый control-api) воркер санитизирует сам по тому же правилу.

**Постепенное раскрытие** ([decisions/progressive-disclosure.md](../decisions/progressive-disclosure.md)).
`disclosure` в спеке: `EAGER` (и `UNSPECIFIED` — старый control-api) — полная спека, как всегда;
`LAZY` — без `input_schema`, с `summary` для листинга; воркер держит такой тул в маршрутной карте,
но не отдаёт модели в `tools`, пока результат `load_tools` не принесёт полную спеку. Маркер
такого тула — `_meta["agimate.context_material"]` со значением `tools` (результат — дельта: `{"tools":
[<ConnectorToolSpec в proto-JSON>], "unknown": [...]}`, воркер применяет её к реестру и отдаёт модели
только имена) или `skill` (результат — тело навыка, уходит модели как есть). Вид результата воркер
пишет в `ToolResultRec.material` при `SaveTurn`, по нему бэк собирает историю следующего рана.
...
ENQUEUED ──первый SaveMessage──► RUNNING ──ANSWER──► DONE
    ...
    │                               └─────ERROR / stale──► FAILED
    ├──ack при cancel_requested_at──► CANCELLED
    └──ack при поглощённом стиринге──► STEERED
```
...
Сообщение, пришедшее в сессию, пока её ран работает, не обязано ждать своей очереди: бегущий ран
забирает его себе и учитывает в текущей работе. Ключевое решение — **забранный ран из очереди DBOS
не вынимается**: диспетчеризация не меняется вовсе, партиция сессии с `concurrency 1` остаётся
единственным арбитром порядка, и гонок здесь нет по построению.

Механика — два RPC и два факта на строке `agent_runs`, потому что захват и поглощение отказывают
по-разному:

| Шаг | RPC | Факт | Потерянный ответ стоит |
|---|---|---|---|
| Захват на шве цикла | `ClaimSteering(agent_id, run_id)` → сообщения младших `ENQUEUED`-ранов сессии | `main_run_id` | ничего: подтверждения не будет, ран отработает сам |
| Подтверждение после вернувшегося LLM-вызова | `MarkSteered(agent_id, run_id, steered_run_ids)` | `steered_at` | дубль ответа — но не потерю сообщения |

Оба вызова — best-effort и **не durable-шаги** (как `SaveTurn`): захват идемпотентен для того же
main (реплей дозабирает захваченное-но-неподтверждённое), чекпоинт формы не меняет.

Когда очередь доходит до забранного рана, его ack seq=0 возвращает `SaveMessageResponse.steered` —
тем же транспортом, что и отмена. Флаг истинен только при **обоих** фактах и терминальном main в
`DONE`/`CANCELLED`; `FAILED`-main ответа пользователю не дал, и ран работает как обычно. Отмена
сильнее стиринга: при обоих флагах ран уходит в `CANCELLED`.

В цикле воркера шов один — перед вызовом модели (диспатч-шов не стирит: между `tool_use` и его
результатами user-ход вставать не может). Поглощение дописывается user-ходом (в журнал — сам текст,
модели — с эфемерным обрамлением «пришло, пока ты работал»), **сбрасывает бюджет ходов** и
перевзводит мягкую посадку, снимая уже вставленный wrap-up-нотис. Сбросов не больше пяти на ран —
дальше шов молчит, и оставшиеся сообщения отрабатывают своим ходом после завершения main.

Забрать можно и событие без prompt-канала: результат отложенного тула, отчёт субагента, напоминание.
Тогда `SteeringMessage.blocks` несёт те же блоки, с которых начался бы его собственный ран (guidance
триггера и основной блок — недоверенный `event` или `trigger_prompt`), воркер рендерит их по правилам
доверия и обрамляет «пришло событие, а не сообщение пользователя». В `text` для события остаётся
компактный JSON — его читает воркер, не знающий поля `blocks`, поэтому порядок выкатки любой.

Подробнее о развилках дизайна — [../decisions/steering.md](../decisions/steering.md).
