...
за финальный ответ, и тул не исполнялся (слабые модели делают это регулярно). Поэтому тул-ход едет
в историю структурно и дробится на две соседние записи:

- `tool_turn{text, calls[]}` — из ASSISTANT-хода, объявившего вызовы; `text` — его преамбула;
- `tool_turn{results[]}` — из следующего за ним TOOL-хода; `text` пуст.

Воркер сшивает их в нативную пару `tool_use`/`tool_result` look-ahead'ом. Осиротевшую results-запись
(calls-половину срезало окном) он отбрасывает — `tool_result` без `tool_use` провайдеры отклоняют.
Симметрично на бэке: незакрытый вызов в конце рана — это негодный журнал, и такой ран из истории
выпадает целиком.

### Целостность журнала

`SaveTurn` — best-effort, и потерянный ход оставил бы `tool_use` без ответа. Журнал проверяется один
раз, когда ран завершается: непрерывность `turn_index` (`count == max + 1`) и парность последнего
хода. Вердикт остаётся в `agent_runs.turns_intact`, и окно истории берёт только раны, где он
положительный, — сборка контекста остаётся чистым чтением. Подробнее —
[../decisions/history-from-turn-ledger.md](../decisions/history-from-turn-ledger.md).
...
- `ANSWER` → answer-канал; в той же транзакции сообщения рана помечаются `completed=true` (видимость
  в интерфейсе), проверяется журнал ходов, и его вердикт остаётся в `agent_runs.turns_intact`. Текст
  пишется в `agent_runs.result` для **любого** рана.
- `ERROR` → progress/answer/prompt-фолбэк, текст в `agent_runs.error`. **ERROR не завершает ран** — в
  историю следующего рана он не попадёт.
...
заметки не должны осесть в завтрашнем контексте. Для direct-рана это вообще единственная строчная
запись вопроса: канальной истории у него нет. `SYSTEM` не пишется никогда — промпт статичен, велик и
уже сохранён в снимке рана.
...
пустой ответ выбрасывается из диалога и турна не даёт, truncation-обрыв — тоже. Поэтому
...
Ран, отменённый **пока стоял в очереди**, выходит сразу после своего ack: контекст не собирается,
в канал не пишется ничего. Статус он получает там же — `INBOUND` при выставленном
`cancel_requested_at` сразу терминален (`CANCELLED`), поскольку следующей записи не будет. Это то, что
не даёт отмене сессии выплюнуть в чат по «остановлено» на каждый ран из очереди.

Терминальная запись рана, который **успел начать работу**, — **`ANSWER`, а не `ERROR`**: остановка не сбой. Отсюда ран
