...
Граница — **drain**: тул-ход, который уже диспатчнут, доводится до конца и записывается обеими
записями, и только следующий ход не начинается. Обрыв посреди хода оставил бы `tool_use` без
`tool_result`, то есть непригодную для следующего рана историю.

Тул **в полёте не прерывается никогда** — запрос, ушедший в Telegram, назад не берётся, и квитанция
в `tool_call_log` важнее секунд. Прерывается только то, что ещё стоит в очереди пула
(`ToolExecutionService.executeAndRecord` отказывается стартовать и пишет обычный failed-результат) —
при пачке параллельных вызовов это как раз большинство.
