...
случаются только при падении процесса на середине рана, а уже сделанные вызовы модели держат свои
чекпоинты сами.

Отсюда два жёстких правила:

- **Смена формы чекпоинта** (DBOS-payload, сигнатуры дочерних workflow) несовместима с in-flight
  ранами → деплой только после **drain**: остановить триггеры, дождаться пустых очередей DBOS.
  Снятие или добавление durable-шага — тоже смена формы: шаги сверяются по паре
  (`function_id`, `function_name`), и сдвиг нумерации ломает реплей уже начатых ранов
