...
- `AcpConnectorService` (`PromptBlockProvider`) отдаёт SYSTEM-блок `ide_session` с путём проекта:
  ...
  Блок появляется только у рана с prompt-каналом, чья сессия есть в registry, то есть у сообщения из
  IDE. Событие в том же разговоре (`tool_completed`, напоминание) блока не получает: тулов IDE у него
  нет.
